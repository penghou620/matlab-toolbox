...
elseif nargin == 0 | ~isstr(action)
   ...
   line_markers = line(xdata,sin(xdata),'Parent',hAxes,'Erasemode','Xor','LineWidth',DefLineWidth,'marker','o');
   xdata = [-pi:0.1:0];
   line_cos = line(xdata,sin(xdata),'Parent',hAxes,'Erasemode','Xor','LineWidth',DefLineWidth);
   %------------------------%
   % Setup Slider
   %------------------------%
